  ...
  {skillsData.map((category) => (
    <div key={category.id} className="space-y-3">
      <div className="flex items-center gap-3">
        <span className={`grid grid-cols-3 gap-0.5 ${colorByTitle[category.title] ?? "text-muted-foreground"}`}>
          {Array.from({ length: 9 }).map((_, i) => (
            <span key={i} className="h-1.5 w-1.5 rounded-full bg-current" />
          ))}
        </span>
        <h3
          className={`text-sm font-extrabold uppercase tracking-wider ${colorByTitle[category.title] ?? "text-muted-foreground"}`}
        >
          {category.title}
        </h3>
      </div>
      ...
      <ul className="flex flex-wrap gap-x-8 gap-y-4">
        {category.skills.map((skill) => (
          <li key={skill.id} className="inline-flex items-center gap-2 text-[15px] font-semibold opacity-90 hover:opacity-100 transition">
            <img
              src={skill.image.src}
              alt=""
